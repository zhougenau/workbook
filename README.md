...
“AI 卡片”会为用户勾选的单词生成学习卡片，每张包含单词、中文释义和一条自然英文用法。首次显示和每次切换卡片时会自动朗读单词与用法，也可以点击朗读按钮重播。若没有勾选任何单词，生成时会从整个单词本公平随机抽取最多 10 个词；重新生成会重新抽取。服务端最多接收 60 个单词，并会验证词条归属和返回卡片是否完整覆盖请求词条。
