...
复习结束后，应用会按难度和题型计算每个单词的学习证据：进阶和挑战题权重高于基础题，填空和实际用法题权重高于直接释义题，答错的负向权重略高。证据达到 `+1.75` 建议提升一级，低于 `-1.75` 建议降低一级，其余保持不变；单轮最多升降一级，并限制在 0–5 级。结果页会展示每个词的答对数、证据值和建议变化，由用户确认后批量保存并同步；同一份测验只能应用一次，避免重复加分。
