...
使用时先登录，在单词库点击“AI 复习”，可以逐个勾选单词，也可以点击“一键全选”选中整个词库，然后选择最低题目数量及难度后生成。实际题数不会少于本轮题源词数，服务端会验证每个目标单词至少被一道题覆盖；例如学习 10 个单词时至少生成 10 道题。应用会先同步所选词条；为控制请求体积和 Token 成本，选择超过 60 个词时，每轮会从所选词池随机抽取 60 个交给 AI。未登录、未同步或不属于当前用户的词条不会被函数读取。
