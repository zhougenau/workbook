...
- 登录后可将所选单词生成为包含单词、释义和英文用法的学习卡片；未选择时随机抽取最多 10 个词
- 0–5 级掌握度管理，默认值为 0
- 搜索、掌握度筛选和多种排序方式
- 编辑、删除、学习统计
- JSON 备份导入和导出
...
在“记下新单词”的“智能选取”框中粘贴包含单词、词性、音标、中文释义和例句的文字，然后点击“智能分析”。应用会自动填写单词、释义和例句字段；释义只截取中文内容，不保留英文词性、音标或英文解释。英文例句既可以使用 `e.g.`、`example` 或“例句”标记，也可以单独成行；无标记例句后一行的中文翻译不会写入例句。分析完成后可以修改各字段，确认无误再点击“收录单词”写入数据库。
...
/ɪmˈperətɪv/
迫切的；必要之事（形容词／名词）
Cutting plastic waste is an environmental imperative.
减少塑料垃圾是环保的当务之急。
```
...
使用时先登录，在单词库点击“AI 复习”，可以逐个勾选单词，也可以点击“一键全选”选中整个词库，然后选择最低题目数量及难度后生成。实际题数不会少于本轮题源词数，服务端会验证每个目标单词至少被一道题覆盖；例如学习 10 个单词时至少生成 10 道题。回答正确后会保留解析 2 秒，再自动进入下一题；回答错误时仍由用户手动继续。应用会先同步所选词条；为控制请求体积和 Token 成本，选择超过 60 个词时，每轮会从所选词池随机抽取 60 个交给 AI。未登录、未同步或不属于当前用户的词条不会被函数读取。

每个词条还提供“AI 对话”按钮。对话框会绑定当前单词，可多轮询问释义、词源、搭配、例句、近义词区别和记忆方案。对话记录仅保留在当前弹窗中，不写入数据库；服务端最多接收最近 24 条消息，每条最多 2000 字符，AI 单次最多输出 2500 Tokens，并会重新验证登录用户及词条归属。
...
“AI 卡片”会为用户勾选的单词生成学习卡片，每张包含单词、中文释义、自然英文用法、同义词、反义词和中文记忆方法。没有可靠反义词时会明确显示暂无常用反义词，不要求 AI 编造。首次显示和每次切换卡片时会自动朗读单词与用法，也可以点击朗读按钮重播。到达最后一张后，可以直接使用本轮实际学习的单词开始 AI 测试，无需重新选择。若没有勾选任何单词，生成时会从整个单词本公平随机抽取最多 10 个词；重新生成会重新抽取。服务端最多接收 60 个单词，并会验证词条归属和返回卡片是否完整覆盖请求词条。

答题页和结果页会显示本次 DeepSeek 调用的输入、输出及总 Token 数。若模型首次返回无效题目并触发自动重试，页面显示的是两次调用的累计用量；这里只显示 Token 数量，不包含 API Key 或其他敏感信息。

复习结束后，应用会按难度和题型计算每个单词的学习证据：进阶和挑战题权重高于基础题，填空和实际用法题权重高于直接释义题，答错的负向权重略高。证据达到 `+1.75` 提升一级，低于 `-1.75` 降低一级，其余保持不变；单轮最多升降一级，并限制在 0–5 级。结果页会展示每个词的答对数、证据值和变化，并在完成测验后自动批量保存和同步；保存失败时可在结果页重试，同一份测验不会重复调整。若本轮存在错题，结果页还会提供“强化复习”按钮，按单词 ID 去重后仅使用上一轮答错的单词生成新题，并沿用当前题量和难度。
